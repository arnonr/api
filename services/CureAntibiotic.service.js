const config = require("../configs/app"),
  ...
  db = require("../models/CureAntibiotic"),
  { Op } = require("sequelize");

const CureAntibioticToAnimalType = require("../models/CureAntibioticToAnimalType");
...
const methods = {
  scopeSearch(req, limit, offset) {
    ...
    if (req.query.CureAntibioticID)
      $where["CureAntibioticID"] = req.query.CureAntibioticID;

    if (req.query.CureAntibioticCode)
      $where["CureAntibioticCode"] = {
        [Op.like]: "%" + req.query.CureAntibioticCode + "%",
      };

    if (req.query.CureAntibioticName)
      $where["CureAntibioticName"] = {
        [Op.like]: "%" + req.query.CureAntibioticName + "%",
      };
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        AnimalTypeIDList.forEach((AnimalTypeID) => {
          const obj1 = CureAntibioticToAnimalType.create({
            CureAntibioticID: inserted.CureAntibioticID,
            AnimalTypeID: AnimalTypeID,
            CreatedUserID: data.CreatedUserID,
          });
        });

        let res = methods.findById(inserted.CureAntibioticID);
  ...
  update(id, data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Update
        data.CureAntibioticID = parseInt(id);
        ...
        if (data.AnimalTypeID) {
          ...
          const searchPTA = await CureAntibioticToAnimalType.findAll({
            where: { CureAntibioticID: obj.CureAntibioticID },
          });
          ...
          AnimalTypeIDList.forEach(async (AnimalTypeID) => {
            const searchPTAOne = await CureAntibioticToAnimalType.findOne({
              where: {
                CureAntibioticID: obj.CureAntibioticID,
                AnimalTypeID: AnimalTypeID,
              },
            });
        ...
        let res = methods.findById(data.CureAntibioticID);
        resolve(res);
      } catch (error) {
        reject(ErrorBadRequest(error.message));
      }
