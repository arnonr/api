...
const methods = {
  scopeSearch(req, limit, offset) {
    ...
    if (req.query.CureHormoneID)
      $where["CureHormoneID"] = req.query.CureHormoneID;

    if (req.query.CureHormoneCode)
      $where["CureHormoneCode"] = {
        [Op.like]: "%" + req.query.CureHormoneCode + "%",
      };

    if (req.query.CureHormoneName)
      $where["CureHormoneName"] = {
        [Op.like]: "%" + req.query.CureHormoneName + "%",
      };
...
data.createdAt = isoDateTime;
        ...
        // insert CureHormoneToAnimalType
        AnimalTypeIDList.forEach((AnimalTypeID) => {
          const obj1 = CureHormoneToAnimalType.create({
            CureHormoneID: inserted.CureHormoneID,
            AnimalTypeID: AnimalTypeID,
            CreatedUserID: data.CreatedUserID,
          });
        });

        let res = methods.findById(inserted.CureHormoneID);
  ...
  update(id, data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Update
        data.CureHormoneID = parseInt(id);
        ...
        if (data.AnimalTypeID === null) {
          CureHormoneToAnimalType.destroy({
            where: {
              CureHormoneID: id,
            },
            // truncate: true,
        ...
        if (data.AnimalTypeID) {
          // insert CureHormoneToAnimalType
          const searchPTA = await CureHormoneToAnimalType.findAll({
            where: { CureHormoneID: obj.CureHormoneID },
          });
          ...
          searchPTA.forEach((pta) => {
            ...
            if (!AnimalTypeIDList.includes(pta.AnimalTypeID)) {
              CureHormoneToAnimalType.destroy({
                where: {
                  CureHormoneToAnimalTypeID: pta.CureHormoneToAnimalTypeID,
                },
              });
            }
          });
          ...
          AnimalTypeIDList.forEach(async (AnimalTypeID) => {
            const searchPTAOne = await CureHormoneToAnimalType.findOne({
              where: {
                CureHormoneID: obj.CureHormoneID,
                AnimalTypeID: AnimalTypeID,
              },
            });
        ...
        let res = methods.findById(data.CureHormoneID);
        resolve(res);
      } catch (error) {
        reject(ErrorBadRequest(error.message));
      }
  ...
  delete(id) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // delete CureHormoneToAnimalType
        const obj1 = CureHormoneToAnimalType.destroy({
          where: { CureHormoneID: id },
          // truncate: true,
        });
