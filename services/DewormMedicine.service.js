const config = require("../configs/app"),
  ...
  db = require("../models/DewormMedicine"),
  { Op } = require("sequelize");

const DMToAnimalType = require("../models/DMToAnimalType");
...
const methods = {
  scopeSearch(req, limit, offset) {
    ...
    if (req.query.DewormMedicineCode)
      $where["DewormMedicineCode"] = {
        [Op.like]: "%" + req.query.DewormMedicineCode + "%",
      };

    if (req.query.DewormMedicineName)
      $where["DewormMedicineName"] = {
        [Op.like]: "%" + req.query.DewormMedicineName + "%",
      };
...
data.createdAt = isoDateTime;
        ...
        // insert DMToAnimalType
        AnimalTypeIDList.forEach((AnimalTypeID) => {
          const obj1 = DMToAnimalType.create({
            DewormMedicineID: inserted.DewormMedicineID,
            AnimalTypeID: AnimalTypeID,
            CreatedUserID: data.CreatedUserID,
          });
        });

        let res = methods.findById(inserted.DewormMedicineID);
  ...
  update(id, data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Update
        data.DewormMedicineID = parseInt(id);
        ...
        await db.update(data, { where: { DewormMedicineID: id } });

        if (data.AnimalTypeID) {
          // insert DMToAnimalType
          const searchPTA = await DMToAnimalType.findAll({
            where: { DewormMedicineID: obj.DewormMedicineID },
          });
          ...
          searchPTA.forEach((pta) => {
            ...
            if (!AnimalTypeIDList.includes(pta.AnimalTypeID)) {
              DMToAnimalType.destroy({
                where: {
                  DMToAnimalTypeID: pta.DMToAnimalTypeID,
                },
              });
            }
          });
          ...
          AnimalTypeIDList.forEach(async (AnimalTypeID) => {
            const searchPTAOne = await DMToAnimalType.findOne({
              where: {
                DewormMedicineID: obj.DewormMedicineID,
                AnimalTypeID: AnimalTypeID,
              },
            });
        ...
        let res = methods.findById(data.DewormMedicineID);
        resolve(res);
      } catch (error) {
        reject(ErrorBadRequest(error.message));
      }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await db.update(
          ...
          { where: { DewormMedicineID: id } }
        );

        // delete DMToAnimalType
        const obj1 = DMToAnimalType.update(
          { isRemove: 1, isActive: 0 },
          { where: { DewormMedicineID: id } }
        );
