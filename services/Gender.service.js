...
const methods = {
  scopeSearch(req, limit, offset) {
    ...
    if (req.query.GenderNameEN)
      $where["GenderNameEN"] = {
        [Op.like]: "%" + req.query.GenderNameEN + "%",
      };
  ...
  insert(data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const res = await db.findByPk(inserted.GenderID);
  ...
  update(id, data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Update
        data.GenderID = parseInt(id);

        data.updatedAt = fn("GETDATE");

        await db.update(data, { where: { GenderID: id } });
