const config = require("../configs/app"),
  ...
  db = require("../models/Vaccine"),
  { Op, fn } = require("sequelize");

const VcToAnimalType = require("../models/VcToAnimalType");
...
const methods = {
  scopeSearch(req, limit, offset) {
    ...
    if (req.query.VaccineCode)
      $where["VaccineCode"] = {
        [Op.like]: "%" + req.query.VaccineCode + "%",
      };

    if (req.query.VaccineName)
      $where["VaccineName"] = {
        [Op.like]: "%" + req.query.VaccineName + "%",
      };
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        AnimalTypeIDList.forEach((AnimalTypeID) => {
          const obj1 = VcToAnimalType.create({
            VaccineID: inserted.VaccineID,
        ...
        let res = methods.findById(inserted.VaccineID);
  ...
  update(id, data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (data.AnimalTypeID) {
          ...
          searchPTA.forEach((pta) => {
            ...
            if (!AnimalTypeIDList.includes(pta.AnimalTypeID)) {
              VcToAnimalType.destroy({
                where: {
                  VcToAnimalTypeID: pta.VcToAnimalTypeID,
                },
              });
            }
          });
          ...
          AnimalTypeIDList.forEach(async (AnimalTypeID) => {
            const searchPTAOne = await VcToAnimalType.findOne({
              where: {
        ...
        let res = methods.findById(data.VaccineID);
        resolve(res);
      } catch (error) {
        reject(ErrorBadRequest(error.message));
      }
