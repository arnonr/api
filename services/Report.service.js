      ...
      try {
        ...
        if (req.query.StaffID) {
          ...
          if (req.query.OrganizationTypeID) {
            $where["OrganizationTypeID"] = Number(req.query.OrganizationTypeID);
          }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!req.query.StaffID) {
          ...
          let organizationIds = organization.map((org) => org.OrganizationID);
        ...
        const queryStaff =
          Object.keys($whereStaff).length > 0 ? { where: $whereStaff } : {};
        ...
        const ai = await AI.findAll({
          attributes: ["ResponsibilityStaffID", "AIID", "AnimalID", "AIDate"],
          ...query,
        ...
        let giveBirth = await GiveBirth.findAll({
          attributes: ["AnimalID", "ResponsibilityStaffID", "ChildGender"],
          where: {
            ResponsibilityStaffID: { [Op.in]: staffIds },
            GiveBirthDate: GiveBirthDate,
            isRemove: 0,
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const endDate = req.query.EndDate
          ? dayjs(req.query.EndDate).format("YYYY-MM-DD")
          : null;
        const startDateCreated = req.query.StartDate_Created
          ? dayjs(req.query.StartDate_Created).format("YYYY-MM-DD")
          : null;
        const endDateCreated = req.query.EndDate_Created
          ? dayjs(req.query.EndDate_Created)
              .endOf("day")
        ...
        if (startDateCreated && endDateCreated) {
          $where["Createddatetime"] = {
            [Op.between]: [startDateCreated, endDateCreated],
          };
        }
                  ...
                  include: [
                    {
                      model: AnimalBreed,
                      as: "AnimalBreed1",
                      attributes: ["AnimalBreedName", "AnimalBreedShortName"],
                    },
                    {
                      model: AnimalBreed,
                      as: "AnimalBreed2",
        ...
        const validRecords = preg.filter(
          (x) => x.AI && x.Animal && x.Animal.AnimalBreedID1 && x.AI.SemenID,
        );
        ...
        validRecords.forEach((record) => {
          ...
          const aiDate = record.AI.AIDate
            ? dayjs(record.AI.AIDate).locale("th").format("DD MMM BB")
            : null;
          const checkupDate = record.CheckupDate
            ? dayjs(record.CheckupDate).locale("th").format("DD MMM BB")
            : null;
          const betweenDate =
            record.CheckupDate && record.AI.AIDate
              ? dayjs(record.CheckupDate).diff(dayjs(record.AI.AIDate), "day")
              : null;

          breedGroup.AnimalID.push({
            AnimalID: record.Animal.AnimalID,
            FarmIdentificationNumber:
              record.Animal.AnimalFarm?.FarmIdentificationNumber,
            ...
            CheckupDate: checkupDate,
            PregnancyCheckStatusName:
              record.PregnancyCheckStatus?.PregnancyCheckStatusName,
            BetweenDate: betweenDate,
            ResponsibilityStaffName:
              `${record.Staff?.StaffGivenName || ""} ${record.Staff?.StaffSurname || ""}`.trim(),
          });
  ...
  report21Old(req) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        $whereStaff["OrganizationProvinceID"] = Number(req.query.ProvinceID);
          ...
          if (ai[i].Animal.AnimalBreedID1 != null && b1 != null) {
            ...
            if (checkBreed) {
              ...
              if (ai[i].PregnancyCheckups.length != 0) {
                let pc =
                  ai[i].PregnancyCheckups[ai[i].PregnancyCheckups.length - 1];
              ...
              checkBreed.AnimalID.push({
                ...
                AnimalBreedAll: ai[i].Animal.toJSON().AnimalBreedAll,
                ...
                GiveBirthDate:
                  ai[i].GiveBirth != null
                    ...
                    : "-",
                ChildGender:
                  ai[i].GiveBirth != null ? ai[i].GiveBirth.ChildGender : "-",
              });
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        breed.forEach((b) => {
          ...
          b.AnimalID.forEach((a) => {
            const key = a.AnimalID;
            if (!uniqueStatus.has(key)) {
              uniqueStatus.set(key, a);
            } else {
              const existing = uniqueStatus.get(key);
              const newDate = new Date(a.AIDateReal);
              const existingDate = new Date(existing.AIDateReal);

              if (a.AnimalPar === existing.AnimalPar) {
                if ((a.TimeNo || 0) > (existing.TimeNo || 0)) {
                  uniqueStatus.set(key, a);
                }
          ...
          const uniqueAnimals = Array.from(uniqueStatus.values());

          b.PregnantCount = uniqueAnimals.filter(
            (a) => a.PregnancyCheckStatusName === "ท้อง",
          ).length;
          b.NotPregnantCount = uniqueAnimals.filter(
            (a) => a.PregnancyCheckStatusName === "ไม่ท้อง",
          ).length;
          b.WaitingForRetestCount = uniqueAnimals.filter(
            (a) => a.PregnancyCheckStatusName === "รอตรวจซ้ำ",
          ).length;
          b.NotCheckedCount = uniqueAnimals.filter(
        ...
        for (let i = 0; i < gbm.length; i++) {
          ...
          if (gb) {
            let check = gbm_arr.findIndex((j) => {
              return j != undefined && j.AnimalID == gbm[i].Animal.AnimalID;
            });
            ...
            if (check == -1) {
              gbm_arr[i] = {
                ...
                FarmProvince: gbm[i].Animal.AnimalFarm?.Province?.ProvinceName,
                FarmAmphur: gbm[i].Animal.AnimalFarm?.Amphur?.AmphurName,
                ...
                GiveBirthDate: gbm[i].GiveBirthDate
                  ? dayjs(gbm[i].GiveBirthDate).locale("th").format("DD MMM BB")
                  : "",
                AIDate: "",
                ResponsibilityStaffName:
      ...
      try {
        ...
        const AnimalID = ai_success.map((instance) => {
          return {
            ...
            AnimalStatusName: instance.Animal.AnimalStatus?.AnimalStatusName,
            AnimalPar: instance.PAR,
            SemenNumber: instance.Semen?.SemenNumber,
            TimeNo: instance.TimeNo,
            ThaiAIDate: instance.AIDate
              ? dayjs(instance.AIDate).locale("th").format("DD MMM BB")
              : "",
              ...
              if (check == -1) {
                ...
                gbm_arr[i] = {
                  ...
                  ThaiAIDate: gbm[i].AI.AIDate
                    ? dayjs(gbm[i].AI.AIDate).locale("th").format("DD MMM BB")
                    : "",
      ...
      try {
        ...
        for (let i = 0; i < gbm.length; i++) {
          ...
          gbm_arr[i] = {
            ...
            AIDate: "",
            ResponsibilityStaffName:
              gbm[i].Staff?.StaffGivenName + " " + gbm[i].Staff?.StaffSurname,
            AIDate: gbm[i].AI?.AIDate,
            ThaiAIDate: gbm[i].AI?.AIDate
              ? dayjs(gbm[i].AI?.AIDate).locale("th").format("DD MMM BB")
              : "",
