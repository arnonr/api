  ...
  insert(data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const res = await db.findByPk(inserted.FarmStatusID);
      ...
      try {
        Promise.all([db.findAll({ ..._q.query, limit: limit, offset: offset })])
          .then(async (result) => {
            ...
            rows = rows.map((data) => {
              let d = {
                FarmStatusID: data.FarmStatusID,
                FarmStatusCode: data.FarmStatusCode,
                FarmStatusName: data.FarmStatusName,
              };
              return d;
            });
