const config = require("../configs/app"),
  ...
  db = require("../models/Disease"),
  { Op } = require("sequelize");

const DsToAnimalType = require("../models/DsToAnimalType");
...
const methods = {
  scopeSearch(req, limit, offset) {
    ...
    if (req.query.DiseaseCode)
      $where["DiseaseCode"] = {
        [Op.like]: "%" + req.query.DiseaseCode + "%",
      };

    if (req.query.DiseaseName)
      $where["DiseaseName"] = {
        [Op.like]: "%" + req.query.DiseaseName + "%",
      };
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        AnimalTypeIDList.forEach((AnimalTypeID) => {
          const obj1 = DsToAnimalType.create({
            DiseaseID: inserted.DiseaseID,
            AnimalTypeID: AnimalTypeID,
            CreatedUserID: data.CreatedUserID,
          });
        });

        let res = methods.findById(inserted.DiseaseID);
      ...
      try {
        ...
        if (data.AnimalTypeID) {
          ...
          searchPTA.forEach((pta) => {
            ...
            if (!AnimalTypeIDList.includes(pta.AnimalTypeID)) {
              DsToAnimalType.destroy({
                where: {
                  DsToAnimalTypeID: pta.DsToAnimalTypeID,
                },
              });
            }
          });
          ...
          AnimalTypeIDList.forEach(async (AnimalTypeID) => {
            const searchPTAOne = await DsToAnimalType.findOne({
              where: {
        ...
        let res = methods.findById(data.DiseaseID);
        resolve(res);
      } catch (error) {
        reject(ErrorBadRequest(error.message));
      }
